...
1. مستند التحليل والتصميم (هذا المستند) + سجلات قرارات المعمارية (ADRs).
2. مخطط قاعدة البيانات (ER) + سكربتات الترحيل (Migrations).
3. **مواصفة OpenAPI/Swagger** (`openapi.yaml`) كعقد رسمي للـ API + Swagger UI.
4. تصميم واجهات (Wireframes / UI Kit).
5. نظام يعمل عبر بيئات: تطوير → اختبار → إنتاج.
6. دليل مستخدم + دليل مدير النظام.
7. خطة نسخ احتياطي واستعادة.
