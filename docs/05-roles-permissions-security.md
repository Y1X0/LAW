...
- **الأرشفة ≠ الحذف:** خلال مدة الاحتفاظ (7 سنوات — [ملف 11](11-quality-and-operations.md))، لا تُحذف سجلات التدقيق إطلاقاً. تقسيم الجداول شهرياً (Partitioning) يُستخدم للأداء و**لأرشفة** الأقسام القديمة إلى تخزين بارد (Cold Storage) لا لحذفها؛ الحذف الفعلي لا يتم إلا **بعد** انقضاء مدة الاحتفاظ القانونية وبموافقة موثّقة.
...
### 5.7 إدارة الأسرار (Secrets Management)
- **لا أسرار في Git إطلاقاً:** `.env` وكل ملفات الأسرار ضمن `.gitignore`؛ يُستخدم `.env.example` بقيم وهمية للتوثيق فقط.
- **مسح الأسرار في CI:** أداة (gitleaks/trufflehog) تفحص كل Push/PR وترفض أي مفتاح/توكن مُسرَّب قبل الدمج.
- **في الإنتاج:** الأسرار في **Secret Manager / HashiCorp Vault** (لا في ملفات على الخادم)، مع تدوير دوري للمفاتيح (Rotation) وصلاحية وصول محدودة.
- **الأسرار الحساسة في قاعدة البيانات** (mfa_secret، auth_token للأجهزة) مشفّرة على مستوى التطبيق (AES-256).
