...
### تفعيل البريد (SMTP) — استعادة كلمة المرور (B2)

مسار «نسيت كلمة المرور» جاهز طرفاً لطرف (خلفية + واجهة). لإرسال البريد **فعليّاً** اضبط الأسرار
التالية في لوحة Render لخدمة `law-api` (كلها `sync:false` في `render.yaml`):
