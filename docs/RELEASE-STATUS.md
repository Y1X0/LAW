...
قنوات إضافية (بريد إلكتروني · Push/Web-Push)، تحديث لحظي (WebSocket/Realtime بدل الجلب عند فتح
النافذة)، قوالب إشعارات قابلة للتخصيص، وربط أحداث المالية (دفعة/فاتورة/عكس) بعد تحديد سياسة
مستلِم واضحة. التفاصيل تُضاف إلى `docs/BACKLOG.md` عند الأولوية.
