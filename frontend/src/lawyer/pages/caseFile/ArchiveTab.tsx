        ...
        <DataTable head={['عنوان الملف', 'الغرفة', 'الخزانة', 'الرف', 'الدرج', 'رقم الملف', 'ملاحظات']}>
          {locations.map((l) => (
            <tr key={l.id} className="border-b border-slate-100 last:border-0">
              <td className="px-4 py-3 font-medium text-slate-800">{l.file_title}</td>
              <td className="px-4 py-3 text-slate-600">{l.archive_room ?? '—'}</td>
              <td className="px-4 py-3 text-slate-600">{l.cabinet ?? '—'}</td>
              <td className="px-4 py-3 text-slate-600">{l.shelf ?? '—'}</td>
              <td className="px-4 py-3 text-slate-600">{l.drawer ?? '—'}</td>
              <td className="px-4 py-3 tabular-nums text-slate-600">{l.file_number ?? '—'}</td>
              <td className="px-4 py-3 text-slate-600">{l.notes ?? '—'}</td>
            </tr>
          ))}
        </DataTable>
      )}
