    ...
    <Modal title="إنشاء فترة رواتب" onClose={onClose}>
      <form onSubmit={onSubmit} className="space-y-3">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <SelectField label="السنة *" value={year} onChange={(e) => setYear(e.target.value)} error={formErrors.fieldError('year')}>
            {Array.from({ length: 6 }, (_, i) => CURRENT_YEAR - 3 + i).map((y) => (
              <option key={y} value={y}>{y}</option>
            ))}
          </SelectField>
          <SelectField label="الشهر *" value={month} onChange={(e) => setMonth(e.target.value)} error={formErrors.fieldError('month')}>
            {MONTHS.map((label, i) => (
              <option key={i + 1} value={i + 1}>{label}</option>
            ))}
          </SelectField>
        </div>
        <SelectField label="الفرع (اختياري)" value={branchId} onChange={(e) => setBranchId(e.target.value)} error={formErrors.fieldError('branch_id')}>
